        ...
        for (int i = 0; i < categories.size(); i++)
        {
            ...
            if (ImGui::Button("Delete"))
            {
                if (categories.size() > 1)
                {
                    ...
                    // Reset category index if it's now out of bounds
                    if (_categoryIndex >= categories.size())
                    {
                        _categoryIndex = 0;
                    }
                }
...
void renderAddTransactionWindow(bool &show_add_transaction_window, Transaction &new_transaction, int &selected_tab_for_new)
{
    ...
    if (ImGui::BeginTabBar("NewTransactionTabs"))
    {
        ...
        if (ImGui::BeginTabItem("Income"))
        {
            ...
            // Bounds check for category index
            if (_categoryIndex >= categories.size())
            {
                _categoryIndex = 0;
            }
            if (ImGui::BeginCombo("##Category", categories.empty() ? "No Categories" : categories[_categoryIndex].c_str()))
            {
                for (int i = 0; i < categories.size(); ++i)
                {
